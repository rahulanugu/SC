import { Component, OnInit } from '@angular/core';
import {Chart} from 'chart.js';

@Component({
  selector: 'app-patient-statistics',
  templateUrl: './patient-statistics.component.html',
  styleUrls: ['./patient-statistics.component.css']
})
export class PatientStatisticsComponent implements OnInit {
  chart = [];
  

  constructor() { }

  ngOnInit() {
    this.chart = new  Chart('lineChart', {
        type: 'bar',
        label: 'Vital Signs',
        data: {
            labels: ['Blood Pressure', 'Sugar Level', 'Heart Rate', 'Temperature', 'Pain Level'],
            datasets: [{
                label: 'Vital Signs',
                data: [12, 19, 3, 5, 2, 3],
                backgroundColor: [
                    'rgba(255, 99, 132, 0.65)',
                    'rgba(54, 162, 235, 0.65)',
                    'rgba(255, 206, 86, 0.65)',
                    'rgba(75, 192, 192, 0.65)',
                    'rgba(153, 102, 255, 0.65)'
                ],
                borderColor: [
                    'rgba(255, 99, 132, 1)',
                    'rgba(54, 162, 235, 1)',
                    'rgba(255, 206, 86, 1)',
                    'rgba(75, 192, 192, 1)',
                    'rgba(153, 102, 255, 1)'
                ],
                borderWidth: 1
            },
            {
                label: '',
                data: [10, 23, 7, 3, 5, 4],
                backgroundColor: [
                    'rgba(255, 99, 132, 0.65)',
                    'rgba(54, 162, 235, 0.65)',
                    'rgba(255, 206, 86, 0.65)',
                    'rgba(75, 192, 192, 0.65)',
                    'rgba(153, 102, 255, 0.65)'
                ],
                borderColor: [
                    'rgba(255, 99, 132, 1)',
                    'rgba(54, 162, 235, 1)',
                    'rgba(255, 206, 86, 1)',
                    'rgba(75, 192, 192, 1)',
                    'rgba(153, 102, 255, 1)'
                ],
                borderWidth: 1

            }]
        },
        options: {
            responsive: true,
            title:{
                display: true,
                text: 'Medical History'
            },
            scales: {
                yAxes: [{
                    ticks: {
                        beginAtZero: true
                    }
                }]
            }

        }

    });
    this.chart = new  Chart('bar', {
        type: 'bar',
        data: {
            labels: ['Red', 'Blue', 'Yellow', 'Green', 'Purple', 'Orange'],
            datasets: [{
                label: '# of Votes',
                data: [12, 50, 3, 5, 2, 3],
                backgroundColor: [
                    'rgba(255, 99, 132, 0.65)',
                    'rgba(54, 162, 235, 0.65)',
                    'rgba(255, 206, 86, 0.65)',
                    'rgba(75, 192, 192, 0.65)',
                    'rgba(153, 102, 255, 0.65)',
                    'rgba(255, 159, 64, 0.65)'
                ],
                borderColor: [
                    'rgba(255, 99, 132, 1)',
                    'rgba(54, 162, 235, 1)',
                    'rgba(255, 206, 86, 1)',
                    'rgba(75, 192, 192, 1)',
                    'rgba(153, 102, 255, 1)',
                    'rgba(255, 159, 64, 1)'
                ],
                borderWidth: 1
            }]
        },
        options: {
            responsive: true,
            title:{
                display: true,
                text: 'Prescription History'
            },
           
            scales: {
                yAxes: [{
                    ticks: {
                        beginAtZero: true
                    }
                }]
            }
        }

    });
    this.chart = new  Chart('bar2', {
        type: 'horizontalBar',
        data: {
            labels: ['Co-Pay Expense for Doctor\'s Visits', 'Out of Pocket Expense', 'Co-pay for Prescription','Out of Pocket Expense for Prescription'],
            datasets: [{
                label: 'Expenses in USD($)',
                data: [75, 19, 5, 25],
                backgroundColor: [
                    'rgba(255, 99, 132, 0.7)',
                    'rgba(54, 162, 235, 0.7)',
                    'rgba(255, 206, 86, 0.7)',
                    'rgba(75, 192, 192, 0.7)'

                ],
                borderColor: [
                    'rgba(255, 99, 132, 1)',
                    'rgba(54, 162, 235, 1)',
                    'rgba(255, 206, 86, 1)',
                    'rgba(75, 192, 192, 1)'
                ],
                borderWidth: 1
            }]
        },
        options: {
            responsive: true,
            title:{
                display: true,
                text: 'Insurance Information'
            },
            scales: {
                yAxes: [{
                    ticks: {
                        beginAtZero: true
                    }
                }]
            }
        }

    });
    this.chart = new  Chart('bar3', {
        type: 'doughnut',
        data: {
            labels: ['Number of Doctor\'s Notes Year to Date'],
            datasets: [{
                label: 'Number of Doctor\'s Notes till today',
                data: [43],
                backgroundColor: [
                    'rgba(54, 162, 235, 0.65)'
                ],
                borderColor: [
                    'rgba(54, 162, 235, 2)',
                ],
                borderWidth: 1
            }]
        },
        options: {
            responsive: true,
            title:{
                display: true,
                text: 'Doctor\'s Note'
            },
            
        }

    });
  }

}
